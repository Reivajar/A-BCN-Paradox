...
Area (km2) & 2.12 & 1.23 & 1.34 & 1.44 \\ \midrule
Density (hab/km2) & 20500 & 34355 & 43200 & 35000 \\ \midrule
...
\caption{MFD of microscopic simulation for the four proposed networks, including demand scaling.}
\label{fig:MFD of microscopic simulation for the four proposed networks.}
\end{figure}


In general terms, the implementation of the superblocks layouts reduces the capacity of the network. We focus on three main metrics: maximum flow ($\Phi_{max}$), density at maximum flow ($d_{\Phi_{max}}$), and optimal speed ($\bar{v}_{opt}$). However, when comparing the network data (table 1) with the results of the simulations (figure 3), we see that this reduction is not linearly proportional to the shrinkage of the typical metrics describing the networks. For the superblocks scenarios, the length of lanes is reduced by 32\% and 36\% and the number of intersections by 75\% and 84\% respectively in networks 2 and 3. However, the maximum capacity of the network 2 drops by 44\% and by 62\% in network 3 (table 5).
