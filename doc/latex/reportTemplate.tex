...
Max. flow ($\Phi_{max}$)(veh./h) & 989 & 555.8 & 372.8 & 438 \\ \midrule
Density at max. flow ($d_{\Phi_{max}}$)(veh./km) & 40.9 & 23.9 & 18.6 & 20.8 \\ \midrule
Optimal speed ($\bar{v}_{opt}$)(km/h) & 24.2 & 23.2 & 19.9 & 21.1 \\ \bottomrule
\end{tabular}%
}
\end{table}

In general terms, the implementation of the superblocks layouts reduces the capacity of the network (see table 5). However, when comparing the network data from table 1 with the results in table 5, we see that this reduction is not linearly proportional to the shrinkage of the typical metrics describing the networks. A reduction in the total length of lanes of 32\% and 36\% respectively in nets 2 and 3 causes a drop of maximum capacity of the networks of 44\% and 62\%. If we consider intersection as the most constraining feature of the networks, the shrinkage is 75\% for network 2 and 84\% for network 3.
More 
